...
class NodoIf(NodoAST):
    ...
    def traducirRuby(self):
        ...
        return res
    def to_dict(self):
        res = {"Tipo": "SentenciaIf", "Condicion": self.condicion.to_dict(), "CuerpoIf": [c.to_dict() for c in self.cuerpo_if]}
        if self.cuerpo_else: res["CuerpoElse"] = [c.to_dict() for c in self.cuerpo_else]
        return res
    def traducirASM(self, ctx):
        asm = ""
...
class NodoWhile(NodoAST):
    ...
    def traducirRuby(self):
        ...
        return res
    def to_dict(self):
        return {"Tipo": "SentenciaWhile", "Condicion": self.condicion.to_dict(), "Cuerpo": [c.to_dict() for c in self.cuerpo]}
    def traducirASM(self, ctx):
        asm = ""
...
class NodoFor(NodoAST):
    ...
    def traducirRuby(self):
        ...
        return res
    def to_dict(self):
        return {"Tipo": "SentenciaFor", "Inicializacion": self.inicializacion.to_dict(), "Condicion": self.condicion.to_dict(), "Incremento": self.incremento.to_dict(), "Cuerpo": [c.to_dict() for c in self.cuerpo]}
    def traducirASM(self, ctx):
...
class NodoFuncion(NodoAST):
    ...
    def traducirPy(self): return f"def {self.nombre[1]}({', '.join(p.traducirPy() for p in self.parametros)}):\n    " + "\n    ".join(c.traducirPy() for c in self.cuerpo)
    def traducirCPP(self): return f"{self.tipo[1]} {self.nombre[1]}({', '.join(p.traducirCPP() for p in self.parametros)}) {{\n    " + ";\n    ".join(c.traducirCPP() for c in self.cuerpo) + ";\n}"
    def traducirGo(self):
        tipo_go = self.tipo[1]
    ...
    def traducirRuby(self): return f"def {self.nombre[1]}({', '.join(p.traducirRuby() for p in self.parametros)})\n    " + "\n    ".join(c.traducirRuby() for c in self.cuerpo) + "\nend"
    def to_dict(self):
        return { "Tipo": "Funcion", "Nombre": self.nombre[1], "Retorno": self.tipo[1], "Parametros": [p.to_dict() for p in self.parametros], "Cuerpo": [c.to_dict() for c in self.cuerpo] }
    def traducirASM(self, ctx):
        asm = f"global {self.nombre[1]}\nextern printf\n{self.nombre[1]}:\n    push rbp\n    mov rbp, rsp\n"
        for c in self.cuerpo: asm += c.traducirASM(ctx)
        asm += "    mov rsp, rbp\n    pop rbp\n    ret\n"
        return asm
...
class Parser:
    ...
    def cuerpo(self):
        ...
        while self.obtener_token_actual() and self.obtener_token_actual()[1] != '}':
            ...
            elif token[0] == 'KEYWORD': instrucciones.append(self.asignacion())
            elif token[0] == 'IDENTIFIER':
                reasig = self.reasignacion()
                self.coincidir('DELIMITER'); instrucciones.append(reasig)
            else: raise SyntaxError(f"Instruccion no reconocida: {token}")
    ...
    def sentencia_while(self):
        self.coincidir('KEYWORD'); self.coincidir('DELIMITER'); condicion = self.expresion()
        self.coincidir('DELIMITER'); self.coincidir('DELIMITER'); cuerpo = self.cuerpo(); self.coincidir('DELIMITER')
        return NodoWhile(condicion, cuerpo)
    def sentencia_for(self):
        self.coincidir('KEYWORD'); self.coincidir('DELIMITER'); inicializacion = self.asignacion(); condicion = self.expresion(); self.coincidir('DELIMITER') 
        incremento = self.reasignacion(); self.coincidir('DELIMITER'); self.coincidir('DELIMITER'); cuerpo = self.cuerpo(); self.coincidir('DELIMITER') 
    ...
    def sentencia_imprimir(self):
        ...
        self.coincidir('DELIMITER'); args = self.lista_argumentos(); self.coincidir('DELIMITER'); self.coincidir('DELIMITER')
    ...
    def asignacion(self):
        tipo = self.coincidir('KEYWORD'); nombre = self.coincidir('IDENTIFIER')
        self.coincidir('OPERATOR'); exp = self.expresion(); self.coincidir('DELIMITER')
        return NodoAsignacion(tipo, nombre, exp)
    def reasignacion(self):
        nombre = self.coincidir('IDENTIFIER'); self.coincidir('OPERATOR'); exp = self.expresion()
    ...
    def expresion(self):
        ...
        while self.obtener_token_actual() and self.obtener_token_actual()[0] == 'OPERATOR':
            op = self.coincidir('OPERATOR'); der = self.termino(); izq = NodoOperacion(izq, op, der)
        return izq
    def termino(self):
        token = self.obtener_token_actual()
        if token[0] == 'NUMBER': return NodoNumero(self.coincidir('NUMBER'))
        if token[0] == 'STRING': return NodoString(self.coincidir('STRING'))
        if token[0] == 'IDENTIFIER': return NodoIdentificador(self.coincidir('IDENTIFIER'))
