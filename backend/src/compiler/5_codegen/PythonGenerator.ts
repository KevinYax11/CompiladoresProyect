...
export class PythonGenerator extends BaseGenerator {
  ...
  protected visitIf(node: ASTNode): string {
    ...
    this.indentLevel++;
    result += n.consequent.length > 0 ? n.consequent.map(child => this.visitNode(child)).join('') : `${this.getIndent()}pass\n`;
    this.indentLevel--;
  ...
  protected visitWhile(node: ASTNode): string {
    ...
    this.indentLevel++;
    result += n.body.length > 0 ? n.body.map(child => this.visitNode(child)).join('') : `${this.getIndent()}pass\n`;
    this.indentLevel--;
